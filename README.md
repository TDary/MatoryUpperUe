...
- **异常分层**：`WidgetNotFoundError` / `CommandError` / `MatoryConnectionError` / `ConnectionKeyError`
- **文件日志**：内置文件日志支持，方便排查和回溯
- **健康监控**：后台线程定时 ping，及时发现断连
...
```python
import matory
print(matory.__version__)  # → "0.1.0"
```
...
```bash
# pytest 方式
pytest autotests/ --matory-host=127.0.0.1 --matory-port=2666 \
    --matory-log-file=matory.log --matory-log-level=DEBUG -v
```
...
with Session("127.0.0.1", 2666, log_file="matory.log", log_level="DEBUG") as s:
    btn = s.find_button(id="1")
    btn.click()
# 所有日志写入 matory.log
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `log_file` | 日志文件路径，`None` 则不写文件 | `None` |
...
page.invalidate_widgets()                  # 清除 Page 缓存的 Widget（切换连接后使用）
session.add_send_hook(callback)            # 注册命令拦截回调
session.remove_send_hook(callback)         # 移除回调
```
...
```python
with Session(host="127.0.0.1", port=2666, timeout=5.0,
             log_file=None, log_level="DEBUG") as s:
    ...
    s.find_text(id="7")                       # → TextWidget (也可用 id/name)
    ...
    s.start_health_check(interval=30.0)        # 启动健康监控
    s.stop_health_check()                      # 停止健康监控
```
...
widget.wait_until(predicate, timeout=10) # → self (等待条件满足)
widget.wait_exists(timeout=10)        # → self (等待出现)
widget.wait_enabled(timeout=10)       # → self (等待启用)
```

### 异常

```python
from matory.errors import MatoryError, CommandError, WidgetNotFoundError, ConnectionKeyError, MatoryConnectionError
...
try:
    session.ping()
except MatoryConnectionError as e:
    print(f"连接异常: {e}")
```
